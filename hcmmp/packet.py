...
class HCMMPPacket:
    def __init__(self, ip, port, host_id: int, flags: int, header_length: int, data_length: int, options: bytes,
                 data: bytes):
        self.ip = ip
        self.port = port
    ...
    def new(host_id: int, flags: int, options: bytes, data: bytes):
        ...
        return HCMMPPacket(None, None, host_id, flags, 8 + len(options), len(data), options, data)
    ...
    def get_raw_packet(self):
        return struct.pack(f"!IBBH{len(self.options)}s{len(self.data)}s", self.host_id, self.flags,
                           self.header_length, self.data_length, self.options, self.data)
...
def parse_hcmmp_packet(packet_bytes: bytes, sender_addr) -> HCMMPPacket:
    ...
    packet_id, flags, header_length, data_length = struct.unpack("!IBBH", packet_bytes[:8])
    ...
    if sender_addr is None or len(sender_addr) != 2:
        return HCMMPPacket(None, None, packet_id, flags, header_length, data_length, options, data)
    else:
        return HCMMPPacket(sender_addr[0], sender_addr[1], packet_id, flags, header_length, data_length, options, data)
